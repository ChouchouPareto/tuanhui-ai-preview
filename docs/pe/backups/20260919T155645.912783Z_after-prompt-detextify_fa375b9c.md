# PE工程快照：after-prompt-detextify

UTC：20260919T155645.912783Z
...
SHA256：b12e50216724332bb7d49d0772b143f540d5bc4d9c82cb8ca0bcae61433c8ad1
...
SHA256：da56d6280f1895131234cfd1596107bd484bc0ef08dbe64e59091a6227b70ba2
...
def inspect_task(db, task):
    ...
    files = {}
    for name in ("model-visual.png", "long.png", "text-guard.json", "generation-audit.json"):
...
def route_reply(db, text, task=None, ambiguous=False):
    ...
    if intent == "status":
        ...
        result["next_steps"] = ["在上方预览这版图片，指出具体文字或区域。", "若只是改标题，可输入「标题改成……」；其他修复先明确范围，不自动整图重做。"]
    elif task.status in {TaskStatus.PENDING, TaskStatus.RUNNING}:
...
SHA256：d2d2e45e37dca9d30cb67797708d380f04ee36e565d3922294289531b9ed6e01
...
def create_design_plan(project_id: str, payload: DesignPlanCreate | None = None, db: Session = Depends(get_db)):
    ...
    item = DesignPlan(project_id=project_id, fact_version=fact.version, version=1 if latest is None else latest.version + 1,
                      plan=build_design_plan(fact.facts, style, asset_count=len(dishes),
...
SHA256：2afc14d12897913ab58aae9e48b814b2a1cfc9d5a659c72f8c75d027d12c52e3
...
SHA256：a5eb007787a17b86a36db72717665ca692ddc00d61d55986a2c778084e3fb66f
...
SHA256：a80c2c8cba6581306dea2de366e07cea51aaea09487365aeea1a9f7cc141e12d
...
SHA256：79dd065ba90c784632c3e32b74e7203209d4bbca0b4bef2c191000ec2ea94308
...
SHA256：54e404c52586cecbf4f73834be4236e969ed7fcbbc24126fee25328634615a09
...
def build_visual_prompt(plan: dict) -> str:
    if plan.get("template_version") == "region-master-v3":
        ...
        # Visual-only style hints: never forward the style display name or keywords such
        # as 「粗体标题」 that the image model may transcribe onto the canvas.
        style_visuals = {
        ...
        instructions += "\n画面主体：" + subject
        instructions += "\n视觉风格：" + style_visuals.get(plan["style"]["key"], style_visuals["appetite"])
        if plan.get("render_mode") != "illustration":
            instructions += "\n本次为真实照片排版：只生成统一背景和装饰，主体视觉区留给本地真实素材，不绘制食物、餐具或额外产品。"
        else:
            instructions += "\n本次为示意设计：按主体视觉区安排主题静物，不等分为五个或三个场景。"
        return instructions
    if plan.get("render_mode") == "illustration":
...
SHA256：7ba1d161c6ff3765b980d8517d6eb3e7eb7f2f70bf73c0783d75ececff323c5c
...
SHA256：7bbc71fc9ff589e285d55c1abc99c3448126c809a7ed3b325d25d83e8dba5944
...
SHA256：71d542888e67b4a240845f32f584fafa1382c8a4f14f01414e9b73637c29546a
...
SHA256：97863fc2bbec0a6834625111d69f2002989488fc10080c2795df5e43be5563fc
...
SHA256：1e744d36a5115d9def63bc1a27373475edef12a2171eed1ed14419ac24c408f9
...
SHA256：0b9f107dcd0fc912c0db204df4f81a9c68d19a485031bf5e79063c6436115f36
...
def compact_layout(layout):
    ...
    """
    labels = {"copy": "文字留白区（低细节背景，供程序排字，不在此处画字）",
              "visual": "主体视觉区",
              "accent": "装饰区"}
    regions = []
    for r in layout["regions"]:
        x, y, w, h = r["box"]
        regions.append(f"{labels.get(r['role'], '区域')}位于画布横向{x:.0%}至{x+w:.0%}、纵向{y:.0%}至{y+h:.0%}")
...
SHA256：6744d26dc2f1d9ada4f4c221b5a4bf95097b90ac8a746dccc23741577ccad51d
