...
## 当前阅读顺序（2026-09-16 整理）

先读[主 PRD](../../PRD.md)与[需求追踪](../product/requirements.json)，再按问题阅读下列合同。本文是导航，不是另一份运行提示词，也不替代模型质量验收。

- 全链路与行业分类：[Input–Output v0.10.1](Input-Output完整PE工程与类目架构审核_v0.10.1.md)；输出标准/交付边界：[v0.11.1](Input-Output完整PE工程与交付核对_v0.11.1.md)。
- 投诉、追问、授权与动作：[意图路由 v0.12.0](对话意图与动作路由_PE设计_2026-09-15_v0.12.0.md)。完整设计与已实现子集请分开看。
- 可编辑文字、坐标、版本与局部修改：[原生对象 v0.13.0](原生对象与局部修改约束_v0.13.0.md)。
- 实际运行文本：[prompts](../../backend/app/services/prompts)。同目录有旧/新版本，实际调用需追踪代码，不能只凭文件名判断。
- 最新设计计划：[专业画布审核](../design/专业画布_小云雀参考拆解与审核计划.md)。父子 Agent、构图库、无限画布已正式纳入，但尚未整体完成；旧阶段“暂缓”不再代表当前范围。

全资料导航位于项目上一级 `00-团绘AI资料导航/README.md`。以下版本记录与备份继续保留，供审核对比。
...
该阶段父子 Agent、构图模板库扩充与无限画布按当时要求暂缓；后续范围已更新，见上方当前阅读顺序。离线测试通过不代表真实模型视觉质量或耗时已通过人工验收。
