...
from app.models import ProjectStatus, SourceAsset, StoreProject, TaskStatus, WorkflowTask
...
def test_running_generation_can_be_paused(client):
    project_id = create_project(client)
    with SessionLocal() as db:
        task = WorkflowTask(project_id=project_id, task_type="group_buying_image_generation", status=TaskStatus.RUNNING, progress=28)
        db.add(task)
        db.commit()
        task_id = task.id
    ...
    assert response.json()["error"]["code"] == "PAUSED_BY_USER"
    with SessionLocal() as db:
        project = db.get(StoreProject, project_id)
        assert project.status == ProjectStatus.DESIGN_PLAN_CONFIRMED
