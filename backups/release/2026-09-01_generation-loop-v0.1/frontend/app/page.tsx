...
type GenerationTask = { id: string; status: "PENDING" | "RUNNING" | "SUCCEEDED" | "FAILED_FINAL"; progress: number; result: { long_image?: string; slices?: string[]; provider?: string; model?: string }; error: { code: string; message: string } | null };
...
function FactsStep({ coverage, busy, onSubmit, onConfirm, onSelectStore, factText }: { coverage: Coverage | null; busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onConfirm: () => void; onSelectStore: (name: string) => void; factText: (key: string) => string }) {
  ...
  return <div className="stepBody factsStep"><div className="factsLayout"><section className="conversationPanel"><div className="panelTitle compactPanelTitle"><span className="assistantAvatar"><BrandMark /></span><div><strong>确认关键信息</strong><small>{coverage.questions.length ? `一次确认一项 · 共 ${coverage.questions.length} 项` : "信息已经齐全"}</small></div></div>
    ...
    {coverage.questions.length > 0 && currentQuestion ? <form className="questionFlow" noValidate onSubmit={handleQuestionSubmit}>{coverage.questions.map((question, index) => index === questionIndex ? null : <input key={question.field} type="hidden" name={question.field} value={answers[question.field] ?? ""} />)}<div className="questionProgress"><span>问题 {questionIndex + 1} / {coverage.questions.length}</span><div aria-hidden="true">{coverage.questions.map((question, index) => <i key={question.field} className={index <= questionIndex ? "active" : ""} />)}</div></div><label htmlFor={`fact-answer-${currentQuestion.field}`}>{currentQuestion.question}</label><div className={`answerComposer ${answerError ? "hasError" : ""}`}><input id={`fact-answer-${currentQuestion.field}`} name={currentQuestion.field} value={answers[currentQuestion.field] ?? ""} placeholder="输入准确答案" aria-invalid={Boolean(answerError)} onChange={(event) => { setAnswers((current) => ({ ...current, [currentQuestion.field]: event.target.value })); if (answerError) setAnswerError(""); }} /><button className="primaryButton" disabled={busy}>{busy ? "正在保存…" : <>{questionIndex === coverage.questions.length - 1 ? "完成确认" : "保存并继续"}<Icon name="arrow" size={17} /></>}</button></div>{answerError && <small className="fieldError" role="alert">{answerError}</small>}{questionIndex > 0 && <button className="questionBack" type="button" onClick={() => { setQuestionIndex((current) => current - 1); setAnswerError(""); }}>返回上一项</button>}</form> : <div className="readyMessage"><Icon name="check" size={20} /><div><strong>核心信息已补齐</strong><p>核对右侧摘要后即可锁定。</p></div></div>}
    </section><aside className="factCard compactFactCard"><div className="factCardHeader"><div><strong>事实摘要</strong><small>V{coverage.fact_version} · 品牌官方视角</small></div><button className="primaryButton" type="button" disabled={!coverage.ready_for_confirmation || busy} onClick={onConfirm}><Icon name="check" size={16} />确认锁定</button></div><dl><div><dt>门店名称</dt><dd>{factText("store_name")}</dd></div><div><dt>门店定位</dt><dd>{factText("positioning")}</dd></div><div><dt>主推内容</dt><dd>{factText("hero_item")}</dd></div><div><dt>核心卖点</dt><dd>{factText("selling_points")}</dd></div></dl>{menuProducts.length > 0 && <details className="menuDisclosure"><summary>已识别 {menuProducts.length} 项菜单内容</summary><div className="menuPreview">{menuProducts.slice(0, 5).map((product, index) => <div key={`${product.name}-${index}`}><span>{product.name}</span><b>{product.price_original ?? "未识别"}</b></div>)}{menuProducts.length > 5 && <small>另有 {menuProducts.length - 5} 项</small>}</div></details>}<p className="factCardHint">{coverage.ready_for_confirmation ? "信息已齐，可以锁定。" : "补完左侧问题后即可锁定。"}</p></aside></div></div>;
}
...
export default function Home() {
  ...
  async function pollGeneration(id: string) { try { const task = await apiRequest<GenerationTask>(`/tasks/${id}`); setGenerationTask(task); if (task.status === "PENDING" || task.status === "RUNNING") { window.setTimeout(() => void pollGeneration(id), 1200); return; } setBusy(false); if (task.status === "SUCCEEDED") notify("长图与五张裁切图已生成，可以下载。", "success"); else notify(task.error?.message || "本次生成失败，请检查模型配置。", "error"); } catch (error) { setBusy(false); notify((error as Error).message, "error"); } }
  async function startGeneration() { if (!designPlan || !window.confirm("即将调用千问图片生成模型并产生费用。确认开始生成吗？")) return; setBusy(true); setGenerationTask(null); try { const result = await apiRequest<{ task_id: string }>(`/projects/${projectId}/generation-runs`, jsonRequest("POST", { provider: "qwen", allow_fallback: true })); setTaskId(result.task_id); setGenerationTask({ id: result.task_id, status: "PENDING", progress: 0, result: {}, error: null }); notify("千问正在生成无文字视觉底图，完成后会自动排版与裁切。", "info"); window.setTimeout(() => void pollGeneration(result.task_id), 700); } catch (error) { setBusy(false); notify((error as Error).message, "error"); } }
      ...
      <div className="contentWrap">
        {activeView === "library" ? <AssetLibrary assets={assets} activeTab={libraryTab} onTabChange={setLibraryTab} onBack={() => navigate("create")} /> : <>
          <section className="workbench" aria-label="门店资料采集工作台">
            ...
            {activeStep === 6 && <GenerateStep projectId={projectId} designPlan={designPlan} task={generationTask} busy={busy} onGenerate={startGeneration} />}
