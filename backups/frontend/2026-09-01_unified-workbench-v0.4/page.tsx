...
function UploadStep({ assets, busy, requiredReady, onSubmit, onAnalyze }: { assets: Asset[]; busy: boolean; requiredReady: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => Promise<boolean>; onAnalyze: (useAI: boolean) => void }) {
  ...
  return <div className="stepBody uploadStep">
    ...
    <form className="uploadComposer" onSubmit={submitUpload} aria-busy={busy}>
      ...
      <section className="classificationPanel"><div className="classificationTitle"><div><span>整理这张图片</span><small>先确认分类，需要时再展开高级设置</small></div><span className="selectedRole"><Icon name={currentRole.icon} size={15} />{currentRole.label}</span></div>
        <label className="roleSelect"><span>图片分类</span><div className="roleSelectControl"><Icon name={currentRole.icon} size={19} /><select name="semantic_role" value={selectedRole} onChange={(event) => setSelectedRole(event.target.value)}>{ROLE_OPTIONS.map((role) => <option key={role.value} value={role.value}>{role.label} · {role.hint}{role.value === suggestedRole ? "（推荐）" : ""}</option>)}</select></div><small>系统根据缺失素材推荐「{ROLE_NAMES[suggestedRole]}」，你可以直接修改。</small></label>
