...
function UploadStep({ mode, assets, busy, onUploadBatch, onAnalyze, onContinue }: { mode: "store" | "dish"; assets: Asset[]; busy: boolean; onUploadBatch: (items: PendingUpload[]) => Promise<boolean>; onAnalyze: (useAI: boolean) => void; onContinue: () => void }) {
  ...
  return <div className="stepBody uploadStep">
    ...
    <section className="batchUploader" aria-busy={busy}>
      ...
      {pendingUploads.length ? <div className="pendingGrid">{pendingUploads.map((item, index) => <article className="pendingCard compact" key={item.id}><div className="pendingThumb"><Image src={item.previewUrl} alt={`待上传图片 ${index + 1}：${item.file.name}`} fill unoptimized sizes="88px" /><button type="button" aria-label={`移除 ${item.file.name}`} onClick={() => removePending(item.id)}><Icon name="close" size={12} /></button><span>{index + 1}</span></div><div className="pendingMeta"><strong title={item.file.name}>{item.file.name}</strong><small>{formatFileSize(item.file.size)}</small></div></article>)}</div> : <label className="batchDropzone" htmlFor={`asset-files-${mode}`}><span className="uploadGlyph"><Icon name="upload" size={20} /></span><strong>点击选择{mode === "store" ? "门店" : "菜品"}图片</strong><small>{mode === "store" ? "门头、环境、Logo 等门店相关图片" : "菜单、招牌菜、普通菜品等图片"}</small></label>}
      <div className="batchFooter"><span>{pendingUploads.length ? `待保存 ${pendingUploads.length} 张` : currentReady ? "素材已保存，可继续下一步" : "选择图片后保存"}</span><div className="saveActions"><button type="button" className="secondaryButton libraryButton" disabled title="素材库将在后续版本开放"><Icon name="folder" size={16} />存储到素材库</button><button type="button" className="primaryButton" disabled={(!pendingUploads.length && !currentReady) || busy} onClick={saveAndContinue}>{busy ? "正在保存…" : <>{currentReady && !pendingUploads.length ? "进入下一步" : mode === "store" ? "确认保存，下一步" : "确认保存，进入确认事实"} <Icon name="arrow" size={18} /></>}</button></div></div>
    </section>
    <div className="assetSection"><div className="subheading"><div><strong>已上传{mode === "store" ? "门店" : "菜品"}素材</strong><small>这里出现图片即代表已保存</small></div><span>{visibleAssets.length} 项</span></div>{visibleAssets.length ? <div className="assetGrid">{visibleAssets.map((asset) => <article className="assetCard" key={asset.id}>{asset.preview_path ? <div className="assetThumb"><Image src={apiUrl(asset.preview_path)} alt={`${mode === "store" ? "门店" : "菜品"}素材：${asset.original_name}`} fill unoptimized sizes="88px" /></div> : <span className="assetIcon"><Icon name={mode === "store" ? "store" : "image"} /></span>}<div className="assetCardBody"><small title={asset.original_name}>{asset.original_name}</small></div></article>)}</div> : <div className="emptyState"><span className="emptyIcon"><Icon name={mode === "store" ? "store" : "image"} /></span><div><strong>还没有已上传素材</strong><p>选择图片并点击“确认保存，下一步”。</p></div></div>}</div>
  </div>;
}
...
export default function Home() {
  ...
  const hasStoreAssets = assets.some((asset) => ["storefront", "environment", "logo"].includes(asset.semantic_role)); const maxStep: Step = coverage ? 4 : hasStoreAssets ? 3 : projectId ? 2 : 1; const factText = (key: string) => Array.isArray(coverage?.facts[key]) ? (coverage?.facts[key] as string[]).join("、") : String(coverage?.facts[key] ?? "待确认");
