...
function BrandLogo() {
  return <a className="brand" href="#workspace" aria-label="团绘AI 首页">
    <BrandMark />
...
function Sidebar({ open, onClose, projectName }: { open: boolean; onClose: () => void; projectName: string }) {
  return <><button className={`sidebarScrim ${open ? "isOpen" : ""}`} aria-label="关闭导航" onClick={onClose} /><aside className={`sidebar ${open ? "isOpen" : ""}`} aria-label="主导航">
    <div className="brandRow"><BrandLogo /><button className="iconButton mobileOnly" aria-label="关闭导航" onClick={onClose}><Icon name="close" /></button></div>
...
function UploadStep({ mode, assets, busy, requiredReady, onUploadBatch, onAnalyze, onContinue }: { mode: "store" | "dish"; assets: Asset[]; busy: boolean; requiredReady: boolean; onUploadBatch: (items: PendingUpload[]) => Promise<boolean>; onAnalyze: (useAI: boolean) => void; onContinue: () => void }) {
  const defaultRole = mode === "store" ? "storefront" : "dish";
  const storeAssets = assets.filter((item) => ["storefront", "environment", "logo"].includes(item.semantic_role));
  const dishAssets = assets.filter((item) => ["menu", "signature_dish", "dish"].includes(item.semantic_role));
  ...
  async function submitBatch() {
    ...
    if (await onUploadBatch(pendingUploads)) {
  ...
  return <div className="stepBody uploadStep">
    <div className="uploadHeader"><div><span className="stepKicker">{mode === "store" ? "门店素材" : "菜品素材"}</span><h2>{mode === "store" ? "上传门店相关图片" : "上传菜单与菜品图片"}</h2><p>{mode === "store" ? "门头、环境和 Logo 可以一起上传，系统统一归入门店素材。" : "菜单、招牌菜和普通菜品可以一起上传，系统统一归入菜品素材。"}</p></div><div className="requirementPills"><span className={currentReady ? "done" : ""}><Icon name={currentReady ? "check" : mode === "store" ? "store" : "image"} size={15} />{mode === "store" ? "门店素材" : "菜品素材"}{currentReady ? "已完成" : "待上传"}</span></div></div>
    <section className="batchUploader" aria-busy={busy}>
      <div className="batchToolbar"><div><strong>待上传{mode === "store" ? "门店" : "菜品"}素材</strong><small>{pendingUploads.length ? `已选择 ${pendingUploads.length} 张` : "选择后可以继续添加或移除"}</small></div><label className="secondaryButton" htmlFor={`asset-files-${mode}`}><Icon name="plus" size={17} />{pendingUploads.length ? "继续添加" : "选择多张图片"}</label><input ref={fileInputRef} className="visuallyHiddenFile" id={`asset-files-${mode}`} type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={chooseFiles} /></div>
      {pendingUploads.length ? <div className="pendingGrid">{pendingUploads.map((item, index) => <article className="pendingCard compact" key={item.id}><div className="pendingThumb"><Image src={item.previewUrl} alt={`待上传图片 ${index + 1}：${item.file.name}`} fill unoptimized sizes="120px" /><button type="button" aria-label={`移除 ${item.file.name}`} onClick={() => removePending(item.id)}><Icon name="close" size={14} /></button><span>{index + 1}</span></div><div className="pendingMeta"><strong title={item.file.name}>{item.file.name}</strong><small>{formatFileSize(item.file.size)}</small></div></article>)}</div> : <label className="batchDropzone" htmlFor={`asset-files-${mode}`}><span className="uploadGlyph"><Icon name="upload" size={23} /></span><strong>点击选择{mode === "store" ? "门店" : "菜品"}图片</strong><small>{mode === "store" ? "门头、环境、Logo 等门店相关图片" : "菜单、招牌菜、普通菜品等图片"}</small></label>}
      <div className="batchFooter"><span>{pendingUploads.length ? `本次将上传 ${pendingUploads.length} 张` : "还没有选择图片"}</span><button type="button" className="primaryButton" disabled={!pendingUploads.length || busy} onClick={submitBatch}>{busy ? "正在批量上传…" : <>确认并上传全部 <Icon name="arrow" size={18} /></>}</button></div>
    </section>
    <div className="assetSection"><div className="subheading"><div><strong>已上传{mode === "store" ? "门店" : "菜品"}素材</strong><small>已成功保存</small></div><span>{visibleAssets.length} 项</span></div>{visibleAssets.length ? <div className="assetGrid">{visibleAssets.map((asset) => <article className="assetCard" key={asset.id}>{asset.preview_path ? <div className="assetThumb"><Image src={apiUrl(asset.preview_path)} alt={`${mode === "store" ? "门店" : "菜品"}素材：${asset.original_name}`} fill unoptimized sizes="132px" /></div> : <span className="assetIcon"><Icon name={mode === "store" ? "store" : "image"} /></span>}<div className="assetCardBody"><small title={asset.original_name}>{asset.original_name}</small></div></article>)}</div> : <div className="emptyState"><span className="emptyIcon"><Icon name={mode === "store" ? "store" : "image"} /></span><div><strong>还没有已上传素材</strong><p>选择图片后统一上传即可。</p></div></div>}</div>
    <div className="actionBar"><div><strong>{mode === "store" ? currentReady ? "门店素材已完成" : "先上传门店相关图片" : requiredReady ? "基础素材已齐，可以开始整理" : "请先上传菜品相关图片"}</strong><small>{mode === "store" ? "下一步继续上传菜单与菜品图片。" : "默认通过对话收集事实，不调用模型。"}</small></div><div className="actionButtons">{mode === "store" ? <button className="primaryButton" type="button" disabled={!currentReady || busy} onClick={onContinue}>下一步：菜品素材 <Icon name="arrow" size={18} /></button> : <><button className="ghostButton" type="button" disabled={!requiredReady || busy} onClick={() => onAnalyze(true)}><Icon name="wand" size={17} />AI 辅助识别</button><button className="primaryButton" type="button" disabled={!requiredReady || busy} onClick={() => onAnalyze(false)}>开始整理 <Icon name="arrow" size={18} /></button></>}</div></div>
  </div>;
}
...
export default function Home() {
  ...
  useEffect(() => { window.scrollTo({ top: 0, behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" }); }, [activeStep]);
  const hasStoreAssets = assets.some((asset) => ["storefront", "environment", "logo"].includes(asset.semantic_role)); const hasDishAssets = assets.some((asset) => ["menu", "signature_dish", "dish"].includes(asset.semantic_role)); const requiredReady = hasStoreAssets && hasDishAssets; const maxStep: Step = coverage ? 4 : hasStoreAssets ? 3 : projectId ? 2 : 1; const factText = (key: string) => Array.isArray(coverage?.facts[key]) ? (coverage?.facts[key] as string[]).join("、") : String(coverage?.facts[key] ?? "待确认");
  ...
  return <div className="appShell"><a className="skipLink" href="#workspace">跳到主要内容</a><Sidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} projectName={projectName} /><main className="mainArea" id="workspace"><header className="topbar"><button className="iconButton menuButton" aria-label="打开导航" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(true)}><Icon name="menu" /></button><div className="announcement"><span>NEW</span><b>团绘AI 对话优先测试版</b><small>先确认事实，再进入成图</small></div><div className="topActions"><span className="tokenBadge"><i /> 默认 0 Token</span><a className="helpButton" href="#guide"><Icon name="help" size={18} />帮助</a></div></header><div className="contentWrap"><section className="workbench" aria-label="门店资料采集工作台"><div className="workbenchTop"><div className="workbenchLabel"><Icon name={activeStep === 1 ? "store" : activeStep === 4 ? "chat" : "upload"} size={18} /><span><b>门店视觉包</b><small>{projectName || "新建项目"}</small></span></div><StepTabs active={activeStep} maxStep={maxStep} onSelect={setActiveStep} /><div className="stepCounter">{activeStep} / 4</div></div>{activeStep === 1 && <CreateStep busy={busy} onSubmit={createProject} />}{activeStep === 2 && <UploadStep mode="store" assets={assets} busy={busy} requiredReady={requiredReady} onUploadBatch={uploadBatch} onAnalyze={analyze} onContinue={() => setActiveStep(3)} />}{activeStep === 3 && <UploadStep mode="dish" assets={assets} busy={busy} requiredReady={requiredReady} onUploadBatch={uploadBatch} onAnalyze={analyze} onContinue={() => undefined} />}{activeStep === 4 && <FactsStep coverage={coverage} busy={busy} onSubmit={submitAnswers} onConfirm={confirm} onSelectStore={selectStoreName} factText={factText} />}<div className={`statusToast ${messageTone}`} role="status" aria-live="polite"><span>{messageTone === "success" ? <Icon name="check" size={17} /> : messageTone === "error" ? <Icon name="close" size={17} /> : <Icon name="spark" size={17} />}</span><p>{message}</p>{taskId && <small>任务 {taskId.slice(0, 8)}</small>}</div></section>{activeStep === 1 && <section className="guideSection" id="guide"><div className="sectionHeading"><div><span className="stepKicker">工作方式</span><h2>少识别一次，多确认一步</h2></div><p>把模型用在真正需要看图的地方，把店名、价格和品牌决定权留给商家。</p></div><div className="guideGrid"><article><span>01</span><Icon name="upload" /><h3>素材按步骤上传</h3><p>先上传门店素材，再上传菜品素材，不需要逐张选择分类。</p></article><article><span>02</span><Icon name="chat" /><h3>缺什么就问什么</h3><p>每轮最多三个问题，默认不调用模型，先让用户给出最准确信息。</p></article><article><span>03</span><Icon name="check" /><h3>事实锁定再生成</h3><p>价格、门店名、Logo 与品牌信息经确认后，才会进入后续视觉流程。</p></article></div></section>}</div></main></div>;
}
