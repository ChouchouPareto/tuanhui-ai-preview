...
def list_assets(project_id: str, db: Session = Depends(get_db)):
    ...
    return [{"id": item.id, "asset_type": item.asset_type, "semantic_role": item.semantic_role, "priority": item.priority, "is_hero": item.is_hero, "original_name": item.original_name, "quality": item.quality, "width": item.width, "height": item.height, "preview_path": f"/projects/{project_id}/assets/{item.id}/content"} for item in assets]
...
def upload_asset(project_id: str, asset_type: str = Form(...), semantic_role: str | None = Form(None), priority: int = Form(100), is_hero: bool = Form(False), file: UploadFile = File(...), db: Session = Depends(get_db)):
    project = require_project(db, project_id)
    ...
    asset = SourceAsset(project_id=project_id, asset_type=normalized_type, semantic_role=role, priority=priority, is_hero=is_hero, original_name=file.filename or "upload", storage_path="", mime_type=file.content_type or "", byte_size=0, sha256="")
    db.add(asset)
    db.flush()
    try:
    ...
    db.commit()
    return {"asset_id": asset.id, "asset_type": asset.asset_type, "semantic_role": asset.semantic_role, "priority": asset.priority, "is_hero": asset.is_hero, "quality": asset.quality, "width": asset.width, "height": asset.height}
...
def update_asset_metadata(project_id: str, asset_id: str, payload: AssetMetadataUpdate, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"asset_id": asset.id, "semantic_role": asset.semantic_role, "priority": asset.priority, "is_hero": asset.is_hero}
