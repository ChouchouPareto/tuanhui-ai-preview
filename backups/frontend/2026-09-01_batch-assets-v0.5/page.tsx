...
function StepTabs({ active, maxStep, onSelect }: { active: Step; maxStep: Step; onSelect: (step: Step) => void }) {
  return <div className="modeTabs" aria-label="创作步骤">{["门店信息", "上传素材", "确认事实"].map((label, index) => { const step = (index + 1) as Step; return <button type="button" key={label} className={active === step ? "active" : ""} aria-current={active === step ? "step" : undefined} disabled={step > maxStep} onClick={() => onSelect(step)}><span>{step}</span>{label}</button>; })}</div>;
}
...
type PendingUpload = { id: string; file: File; previewUrl: string; role: string; priority: number; isHero: boolean };

function UploadStep({ assets, busy, requiredReady, onUploadBatch, onAnalyze }: { assets: Asset[]; busy: boolean; requiredReady: boolean; onUploadBatch: (items: PendingUpload[]) => Promise<boolean>; onAnalyze: (useAI: boolean) => void }) {
  ...
  function chooseFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    const existingRoles = new Set([...assets.map((item) => item.semantic_role), ...pendingUploads.map((item) => item.role)]);
    const additions = files.map((file, index) => {
      let role = "signature_dish";
      if (!existingRoles.has("menu")) { role = "menu"; existingRoles.add("menu"); }
      else if (!existingRoles.has("storefront")) { role = "storefront"; existingRoles.add("storefront"); }
      return { id: `${Date.now()}-${index}-${file.name}`, file, previewUrl: URL.createObjectURL(file), role, priority: 100, isHero: false };
    });
    setPendingUploads((current) => [...current, ...additions]);
    event.target.value = "";
  }

  function updatePending(id: string, update: Partial<Pick<PendingUpload, "role" | "priority" | "isHero">>) {
    setPendingUploads((current) => current.map((item) => item.id === id ? { ...item, ...update } : update.isHero ? { ...item, isHero: false } : item));
  }
  ...
  return <div className="stepBody uploadStep">
    <div className="uploadHeader"><div><span className="stepKicker">批量素材采集</span><h2>一次选图，逐张标注</h2><p>支持同时选择最多 20 张图片；缩略图只用于核对，不会挤占整个页面。</p></div><div className="requirementPills"><span className={hasMenu ? "done" : ""}><Icon name={hasMenu ? "check" : "image"} size={15} />菜单{hasMenu ? "已完成" : "待上传"}</span><span className={hasStorefront ? "done" : ""}><Icon name={hasStorefront ? "check" : "store"} size={15} />门头{hasStorefront ? "已完成" : "待上传"}</span></div></div>
    <section className="batchUploader" aria-busy={busy}>
      <div className="batchToolbar"><div><strong>待上传素材</strong><small>{pendingUploads.length ? `已选择 ${pendingUploads.length} 张，逐张确认分类后统一上传` : `优先建议补充：${ROLE_NAMES[suggestedRole]}`}</small></div><label className="secondaryButton" htmlFor="asset-files"><Icon name="plus" size={17} />{pendingUploads.length ? "继续添加" : "选择多张图片"}</label><input ref={fileInputRef} className="visuallyHiddenFile" id="asset-files" type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={chooseFiles} /></div>
      {pendingUploads.length ? <div className="pendingGrid">{pendingUploads.map((item, index) => <article className="pendingCard" key={item.id}><div className="pendingThumb"><Image src={item.previewUrl} alt={`待上传图片 ${index + 1}：${item.file.name}`} fill unoptimized sizes="160px" /><button type="button" aria-label={`移除 ${item.file.name}`} onClick={() => removePending(item.id)}><Icon name="close" size={15} /></button><span>{index + 1}</span></div><div className="pendingMeta"><strong title={item.file.name}>{item.file.name}</strong><small>{formatFileSize(item.file.size)}</small></div><label className="pendingRole"><span>分类</span><select value={item.role} onChange={(event) => updatePending(item.id, { role: event.target.value })}>{ROLE_OPTIONS.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select></label><details className="pendingAdvanced"><summary>更多设置</summary><div><select aria-label={`${item.file.name} 的重要性`} value={item.priority} onChange={(event) => updatePending(item.id, { priority: Number(event.target.value) })}><option value="50">重点素材</option><option value="100">普通素材</option><option value="200">备用素材</option></select><label><input type="checkbox" checked={item.isHero} onChange={(event) => updatePending(item.id, { isHero: event.target.checked })} />唯一主推图</label></div></details></article>)}</div> : <label className="batchDropzone" htmlFor="asset-files"><span className="uploadGlyph"><Icon name="upload" size={23} /></span><strong>点击选择多张门店图片</strong><small>菜单、门头、菜品、环境和 Logo 可以一起选择</small></label>}
...
export default function Home() {
  ...
  async function uploadBatch(items: PendingUpload[]): Promise<boolean> { if (!projectId || !items.length) return false; setBusy(true); let completed = 0; try { for (const item of items) { const fd = new FormData(); fd.append("file", item.file); fd.append("asset_type", "other"); fd.append("semantic_role", item.role); fd.append("priority", String(item.priority)); if (item.isHero) fd.append("is_hero", "true"); await apiRequest(`/projects/${projectId}/assets`, { method: "POST", body: fd }); completed += 1; } await refreshAssets(); notify(`已成功上传 ${completed} 张素材，并保存各自分类。`, "success"); return true; } catch (error) { if (completed) await refreshAssets(); notify(`${completed ? `已完成 ${completed} 张；` : ""}${(error as Error).message}`, "error"); return false; } finally { setBusy(false); } }
