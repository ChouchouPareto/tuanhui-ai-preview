...
function UploadStep({ mode, assets, busy, onUploadBatch, onAnalyze, onContinue, onOpenLibrary }: { mode: "store" | "dish"; assets: Asset[]; busy: boolean; onUploadBatch: (items: PendingUpload[]) => Promise<boolean>; onAnalyze: (useAI: boolean) => void; onContinue: () => void; onOpenLibrary: () => void }) {
  ...
  return <div className="stepBody uploadStep">
    ...
    <section className="batchUploader" aria-busy={busy}>
      <div className="batchToolbar"><div><strong>待上传{mode === "store" ? "门店" : "菜品"}素材</strong><small>{pendingUploads.length ? `已选择 ${pendingUploads.length} 张，保存后自动进入下一步` : currentReady ? "素材已保存，可直接进入下一步" : "选择后可以继续添加或移除"}</small></div><div className="toolbarActions"><label className="secondaryButton chooseButton" htmlFor={`asset-files-${mode}`}><Icon name="plus" size={16} />{pendingUploads.length ? "继续添加" : "选择图片"}</label><button type="button" className="primaryButton topContinueButton" disabled={(!pendingUploads.length && !currentReady) || busy} onClick={saveAndContinue}>{busy ? "正在保存…" : <>{currentReady && !pendingUploads.length ? "进入下一步" : mode === "store" ? "确认保存，下一步" : "确认保存，进入确认事实"} <Icon name="arrow" size={17} /></>}</button><button type="button" className="libraryMiniButton" onClick={onOpenLibrary} aria-label="打开素材库"><Icon name="folder" size={15} />素材库</button></div><input ref={fileInputRef} className="visuallyHiddenFile" id={`asset-files-${mode}`} type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={chooseFiles} /></div>
...
export default function Home() {
  ...
  const hasStoreAssets = assets.some((asset) => STORE_ASSET_ROLES.includes(asset.semantic_role)); const maxStep: Step = coverage ? 4 : hasStoreAssets ? 3 : projectId ? 2 : 1; const factText = (key: string) => Array.isArray(coverage?.facts[key]) ? (coverage?.facts[key] as string[]).join("、") : String(coverage?.facts[key] ?? "待确认");
  function notify(text: string, tone: "info" | "success" | "error" = "info") { setMessage(text); setMessageTone(tone); }
  async function createProject(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); const fd = new FormData(event.currentTarget); const name = String(fd.get("name") ?? "").trim(); try { const result = await apiRequest<{ project_id: string }>("/projects", jsonRequest("POST", { name, industry: "餐饮", platforms: ["douyin", "meituan"] })); setProjectId(result.project_id); setProjectName(name); setAssets([]); setCoverage(null); setActiveView("create"); setActiveStep(2); notify("项目已创建，请先上传门店素材。", "success"); } catch (error) { notify((error as Error).message, "error"); } finally { setBusy(false); } }
    ...
    <main className="mainArea" id="workspace">
      ...
      <div className="contentWrap">
        {activeView === "library" ? <AssetLibrary assets={assets} activeTab={libraryTab} onTabChange={setLibraryTab} onBack={() => navigate("create")} /> : <>
          ...
          {activeStep === 1 && <section className="guideSection" id="guide"><div className="sectionHeading"><div><span className="stepKicker">工作方式</span><h2>少识别一次，多确认一步</h2></div><p>把模型用在真正需要看图的地方，把店名、价格和品牌决定权留给商家。</p></div><div className="guideGrid"><article><span>01</span><Icon name="upload" /><h3>素材按步骤上传</h3><p>先上传门店素材，再上传菜品素材，不需要逐张选择分类。</p></article><article><span>02</span><Icon name="chat" /><h3>缺什么就问什么</h3><p>每轮最多三个问题，默认不调用模型，先让用户给出最准确信息。</p></article><article><span>03</span><Icon name="check" /><h3>事实锁定再生成</h3><p>价格、门店名、Logo 与品牌信息经确认后，才会进入后续视觉流程。</p></article></div></section>}
        </>}
      </div>
    </main>
  </div>;
