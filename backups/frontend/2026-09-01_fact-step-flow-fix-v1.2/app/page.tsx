...
function CreateStep({ busy, onSubmit }: { busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="stepBody createStep"><div className="stepIcon"><Icon name="store" size={27} /></div><div className="stepCopy"><span className="stepKicker">门店项目</span><h2>今天要为哪家店创作？</h2><p>选择门店类型并填写名称，先建立这次设计任务。</p></div><form className="projectForm" onSubmit={onSubmit}><fieldset className="categoryPicker"><legend>门店类型</legend><div role="radiogroup" aria-label="选择门店类型"><label className="active"><input type="radio" name="industry" value="餐饮" defaultChecked /><span><Icon name="store" size={15} />餐饮美食</span></label><label className="comingSoon"><input type="radio" name="industry" value="休闲娱乐" disabled /><span>休闲娱乐<small>暂未开放</small></span></label><label className="comingSoon"><input type="radio" name="industry" value="丽人健身" disabled /><span>丽人健身<small>暂未开放</small></span></label></div></fieldset><label htmlFor="project-name">门店名称</label><div className="inputAction"><input id="project-name" name="name" required maxLength={120} placeholder="例如：山城酸菜鱼" autoComplete="organization" /><button className="primaryButton" disabled={busy}>{busy ? "正在创建…" : <>创建项目 <Icon name="arrow" size={18} /></>}</button></div><div className="formMeta"><span><Icon name="check" size={15} /> 抖音 + 美团</span><span><Icon name="check" size={15} /> 创建项目不消耗 Token</span></div></form></div>;
}
