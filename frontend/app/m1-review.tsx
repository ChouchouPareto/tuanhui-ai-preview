...
export function M1Review({ autoGenerate, onGenerationActive, target, controller, projectId, seed, draft, prepare, onRestore, onReply, onAssets, onBusy }: Props) {
  ...
  return createPortal(<section id="inline-confirmation" tabIndex={-1} className="inlineReview inlineFeedback" aria-label="本次生成摘要" aria-busy={busy}>
    ...
    {task && ["PENDING","RUNNING"].includes(task.status) && <div className="studioProgress" role="status"><h2>正在生成整张长图</h2><p>完成后会自动排版，再切成五张。</p><progress max={100} value={task.progress} /><p>已发送的模型请求可能仍会计费；停止后不再继续后续处理。</p></div>}
    {review?.status === "CONFIRMED" && <div role="status"><p>{task?.error?.message || (task?.status === "SUCCEEDED" ? "作品已生成，可以下载。" : task?.status === "RUNNING" ? "正在生成，离开页面不会取消任务。" : task?.status === "NEEDS_USER" ? "已停止，需求和素材已保留。需要重新生成时请开始下一次创作。" : task?.status === "FAILED_FINAL" ? "这次生成没有完成，需求已保留，没有自动重试。" : "任务已保存，等待后台执行。")}</p>{task?.result.long_image && <a href={apiUrl(`/projects/${projectId}/generations/${task.id}/assets/${task.result.long_image}`)} download><Image width={4000} height={600} unoptimized className="m1Result" src={apiUrl(`/projects/${projectId}/generations/${task.id}/assets/${task.result.long_image}`)} alt="完整五连图，点击下载" /></a>}{task?.result.slices && <div className="m1Options">{task.result.slices.map((file, i) => <a key={file} href={apiUrl(`/projects/${projectId}/generations/${task.id}/assets/${file}`)} download><Image width={800} height={600} unoptimized src={apiUrl(`/projects/${projectId}/generations/${task.id}/assets/${file}`)} alt={`第 ${i + 1} 张切片`} />下载第{i + 1}张</a>)}</div>}<div className="studioResultActions"><a className="primaryButton" href={`/?project=${projectId}&compose=1`}>继续下一次创作</a><a href={`/projects?project=${projectId}`}>返回项目</a></div></div>}
  </section>, target);
}
