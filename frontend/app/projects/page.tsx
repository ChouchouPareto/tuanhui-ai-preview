...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    const request = id ? Promise.all([apiRequest<Workspace>(`/projects/${id}/workspace`), apiRequest<Asset[]>(`/projects/${id}/assets`)]).then(([w, a]) => { if (!disposed) { setWorkspace(w); setAssets(a); } }) : apiRequest<Project[]>(`/projects?visibility=${visibility}`).then(p => { if (!disposed) setProjects(p); });
  ...
  return <main className="projectHub">
    ...
    {!workspace && <nav className="projectTabs" aria-label="项目分类">{[["visible","我的项目"],["hidden","已隐藏"],["deleted","回收站"]].map(([key,label])=><button key={key} aria-pressed={visibility === key} onClick={()=>setVisibility(key)}>{label}</button>)}</nav>}
