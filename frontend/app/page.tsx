...
function QuickCreationHome({ projectId, projectName, assets, coverage, generationTask, designPlan, busy, message, messageTone, onSubmit, onSubmitFacts, onConfirmAndGenerate, onGenerateExisting, onPause }: { projectId: string; projectName: string; assets: Asset[]; coverage: Coverage | null; generationTask: GenerationTask | null; designPlan: DesignPlan | null; busy: boolean; message: string; messageTone: "info" | "success" | "error"; onSubmit: (payload: ConversationIntakePayload) => Promise<boolean>; onSubmitFacts: (event: FormEvent<HTMLFormElement>) => void; onConfirmAndGenerate: (style: string, model: string) => void; onGenerateExisting: () => void; onPause: () => void }) {
  const [brief, setBrief] = useState("");
  const [storeItems, setStoreItems] = useState<PendingUpload[]>([]);
  const [dishItems, setDishItems] = useState<PendingUpload[]>([]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!brief.trim()) { setError("请先描述门店名称、主推内容、价格和真实卖点"); return; }
    if (!name) { setError("请在文字中写明“门店名称：×××”"); return; }
    if (!savedStoreCount && !storeItems.length) { setError("请添加至少一张门店素材"); return; }
    if (!savedDishCount && !dishItems.length) { setError("请添加至少一张菜品或菜单素材"); return; }
    setError("");
    const saved = await onSubmit({ name, brief: `${brief.trim()}\n视觉风格：${style}\n排版布局：${layout}\n模板：${template}\n模型：${model}`, storeItems, dishItems });
  ...
  return <section className="quickCreationHome" aria-labelledby="quick-creation-title">
    <header className="creationAgentHeader"><div className="creationTypeTabs" role="tablist" aria-label="创作模式"><button type="button" role="tab" aria-selected="false" disabled>创作</button><button type="button" className="active" role="tab" aria-selected="true">一键生图</button><button type="button" role="tab" aria-selected="false" disabled>营销方案</button></div><h1 id="quick-creation-title">让每家门店，都有一套会成交的设计</h1></header>
    ...
    {coverage && !generationTask && designPlan?.status !== "CONFIRMED" && <QuickFactConfirmation key={`${coverage.fact_version}-${coverage.questions.map((question) => question.field).join("|")}`} coverage={coverage} busy={busy} onSubmit={onSubmitFacts} onConfirmAndGenerate={() => onConfirmAndGenerate(style, model)} />}
    {(designPlan?.status === "CONFIRMED" || generationTask) && <div className="quickGenerationPanel"><GenerateStep projectId={projectId} designPlan={designPlan} task={generationTask} busy={busy} onGenerate={onGenerateExisting} onPause={onPause} /></div>}
...
export default function Home() {
  ...
  return <div className="appShell">
    ...
    <main className="mainArea" id="workspace">
      <header className="topbar"><button className="iconButton menuButton" aria-label="打开导航" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(true)}><Icon name="menu" /></button><div className="announcement"><span>NEW</span><b>团绘AI 一键生图首页</b><small>首页确认真实信息，直接成图</small></div><div className="topActions"><span className="tokenBadge"><i /> 默认 0 Token</span><a className="helpButton" href="#guide"><Icon name="help" size={18} />帮助</a></div></header>
      <div className="contentWrap">
        {activeView === "library" ? <AssetLibrary assets={assets} activeTab={libraryTab} onTabChange={setLibraryTab} onBack={() => navigate(libraryReturnView.current)} /> : activeView === "oneclick" ? <QuickCreationHome projectId={projectId} projectName={projectName} assets={assets} coverage={coverage} generationTask={generationTask} designPlan={designPlan} busy={busy} message={message} messageTone={messageTone} onSubmit={submitConversationIntake} onSubmitFacts={submitAnswers} onConfirmAndGenerate={confirmQuickAndGenerate} onGenerateExisting={startGeneration} onPause={pauseGeneration} /> : <>
