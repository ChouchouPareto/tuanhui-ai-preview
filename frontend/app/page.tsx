...
function ConversationIntake({ projectName, assets, busy, onSubmit }: { projectName: string; assets: Asset[]; busy: boolean; onSubmit: (payload: ConversationIntakePayload) => Promise<boolean> }) {
  ...
  return <form className="singleConversationComposer" noValidate onSubmit={submit}>
    ...
    <label className="conversationBriefField"><span className="srOnly">本次创作需求</span><textarea value={brief} maxLength={800} disabled={busy} placeholder={`描述你想要的团购设计，并补充真实信息，例如：\n门店定位：川渝江湖菜　主推套餐：酸菜鱼双人餐\n真实价格：99 元　真实卖点：活鱼现做，酸香开胃`} onChange={(event) => setBrief(event.target.value)} /></label>
