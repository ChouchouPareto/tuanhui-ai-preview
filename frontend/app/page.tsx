...
function DesignPlanStep({ designPlan, busy, onUpdate, onConfirm }: { designPlan: DesignPlan | null; busy: boolean; onUpdate: (style: string) => void; onConfirm: () => void }) {
  ...
  return <div className="stepBody designPlanStep"><header className="designPlanHeader"><div><span className="stepKicker">五图设计方案</span><h2>先确认怎么画，再开始生图</h2><p>五屏在同一张 20:3 长图上设计，输出时准确裁成五张 4:3 图片。</p></div><span className="planVersion">方案 V{designPlan.version}</span></header>
    ...
    <footer className="designPlanActions"><p><Icon name="check" size={16} />切换方案只更新预览；确认后才会解锁付费生图。</p><div className="planActionCluster"><details className="planDropdown" ref={layoutMenuRef}><summary><span><small>排版风格</small><b>内容均衡</b></span><i aria-hidden="true" /></summary><div className="planDropdownMenu"><button type="button" className="active" onClick={() => layoutMenuRef.current?.removeAttribute("open")}><span><b>内容均衡</b><small>五屏信息均匀分配</small></span><Icon name="check" size={15} /></button><button type="button" disabled><span><b>爆款主图</b><small>模板库接入后开放</small></span></button><button type="button" disabled><span><b>品牌叙事</b><small>模板库接入后开放</small></span></button></div></details><details className="planDropdown" ref={styleMenuRef}><summary><span><small>视觉风格</small><b>{designPlan.plan.style.name}</b></span><i aria-hidden="true" /></summary><div className="planDropdownMenu">{DESIGN_STYLES.map((style) => <button key={style.key} type="button" className={designPlan.plan.style.key === style.key ? "active" : ""} disabled={busy} onClick={() => { styleMenuRef.current?.removeAttribute("open"); onUpdate(style.key); }}><span><b>{style.name}</b><small>{style.hint}</small></span>{designPlan.plan.style.key === style.key && <Icon name="check" size={15} />}</button>)}</div></details><button className="primaryButton" type="button" disabled={busy} onClick={onConfirm}>{busy ? "正在确认…" : <>确认方案 <Icon name="arrow" size={17} /></>}</button></div></footer>
  </div>;
}
