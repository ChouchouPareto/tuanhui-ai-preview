...
餐饮商家上传菜单、门头及菜品图片，AI提取并让用户确认真实经营信息，再生成一套可编辑、可校验、可直接用于抖音团购与美团团购的“门店视觉包”，包含首页五图、Logo和品牌介绍图。
...
| V0.6 | 2026-08-28 | 需求修正版 | 明确BD仅为种子用户群体，不构成独立产品角色，不开发BD专属入口、工作台或业务通道 |
| V0.7 | 2026-08-31 | 竞品启发优化版 | 吸收已确认的万相营造拆解：确立“门店视觉包”、素材角色与优先级、表达视角、套装式交付和项目内局部返工；同步餐饮首发及千问/豆包模型路由决策 |
...
12. 【已确认】种子用户为BD经理；前期以Web/WAP发布，完成后迁移为小程序形态。BD仅代表首批验证用户，不新增专属产品通道。
13. 【已确认】MVP包含直接发布到目标平台；若官方接口或授权暂不可用，不能把模拟成功当作已发布。
14. 【已确认】趋势采集前期先完成自动采集入口与人工上传后台工作台，正式数据源、调度策略和自动启用时间后置决策。
15. 【已确认】项目正式名称为“团绘AI”。
16. 【已确认】MVP首发行业仅为餐饮，不同时覆盖美业、休闲娱乐等其他行业。
17. 【已确认】OCR与视觉理解首期采用阿里云百炼；图片生成同时接入千问与豆包，默认调用千问，单次失败后第二次可切换豆包；同一次生成不并行调用两家。
18. 【已确认】豆包图片生成以当前控制台可用的`Doubao-Seedream-5.0-lite`为失败切换目标，具体版本ID通过配置管理，不继续依赖已不可见的Seedream 4.0。
19. 【已确认】万相营造竞品拆解结论已获创作者认可；本版本只吸收任务化入口、结构化输入、素材优先级、成套交付和局部返工等机制，不照搬其通用工具数量。
20. 【分析资料】[万相营造竞品拆解V0.1](/Users/pareto.chouchou/Desktop/PRD总文件夹/本地生活全套设计图/团绘AI/竞品拆解_万相营造_v0.1.md)。
...
| BD经理（种子用户群体） | 首批使用团绘AI验证真实门店项目 | 商家素材不全、沟通轮次多、上线交付慢 | 直接使用统一产品完成补问、确认、出图和发布，不需要专属入口 |
...
【已确认】用户完成一次素材上传和事实确认后，至少获得一套“门店视觉包”：
...
> 从店铺原始素材到双平台可用门店视觉包，一次分析、事实先确认、整套生成、局部可改、规则可验。

对外一句话定位：

> 团绘AI——面向本地生活业务使用者的团购视觉生成与发布工具。
...
| 已确认 | 种子用户 | BD经理是首批验证人群，但不是独立系统角色；与其他用户共用统一产品通道 |
| 已确认 | MVP首发行业 | 仅餐饮；其他本地生活行业不进入首版行业选择器 |
| 已确认 | OCR/视觉理解供应商 | 阿里云百炼，生产使用具体版本ID而非漂移的`latest`别名 |
| 已确认 | 生图供应商与切换 | 千问默认；该次千问调用失败后，下一次重试可切豆包Seedream 5.0 Lite；单次不并行双调用 |
...
| 维度 | 已确认事实 | 成功/边界 | 未确认项 |
| --- | --- | --- | --- |
| 目标 | 把餐饮门店原始素材转成门店视觉包并发布 | 产物通过事实、审美、平台规则校验且获得平台可查询状态 | 商业化目标与收入指标 |
| 用户 | 种子用户为BD经理；BD仅是运营意义上的首批用户群体 | 所有用户共用统一Web/WAP入口；关键事实和发布需人在环 | 单账号目标并发项目数 |
...
3. 以“门店视觉包”统一交付和管理五图、Logo、品牌介绍图及相关说明，而不是让用户分别发起多个生图任务。
4. 支持非设计用户通过换图、改字、改色、重新裁切和局部重做完成修改。
5. 让代运营能够保存项目、复用模板并快速交付多个商家。
6. 让用户显式标记门头、Logo、招牌菜、环境等素材角色和优先级，并确认主推卖点与表达视角。
...
| 万相营造 | 按商品图/页面/文案/工具箱组织任务；链接解析与人工上传并存；商品卖点、人设、素材优先级结构化；整套图片和文案交付 | 庞大的通用电商工具目录、隐式依赖图片顺序、首版复杂算力货币和与餐饮五图无关的功能 |
...
6. 支持局部可控修改；
7. 输出前完成规则与内容一致性校验。
8. 用户面对的是“五图/Logo/品牌介绍图/完整视觉包”等经营交付物，而不是文生图、图生图或模型名称。
9. 完整生成与局部返工共用同一门店事实卡，修改一个节点不要求用户重新开始整个项目。
...
```mermaid
flowchart LR
    A[上传菜单、门头与菜品素材] --> A1[标记素材角色与优先级]
    A1 --> B[AI识别与质量检查]
    B --> B1{核心信息是否充分}
    B1 -->|否| B2[第二轮定向追问/补图]
    B2 --> B
    B1 -->|是| C{用户确认门店事实卡}
    C -->|修改后确认| D[选择主推卖点与表达视角]
    D --> D0[品牌与五图策划]
    D0 --> D1[生成五图方案与生图提示词]
...
| P0 | 素材上传 | 上传菜单、门头和可选菜品/环境/Logo图，标记角色与优先级 | 是 |
| P0 | AI分析 | OCR、视觉识别、素材质量检查、字段置信度 | 是 |
| P0 | 缺口追问 | 判断核心信息充分度，按缺失字段发起第二轮对话或补图 | 是 |
| P0 | 门店事实卡 | 用户确认店名、品类、菜品、价格、主推卖点及素材证据 | 是 |
| P0 | 表达方案 | 选择品牌官方/老板推荐/食客种草等表达视角 | 是，提供默认推荐 |
| P0 | 五图策划 | 固定PE约束下生成信息结构、素材分配、排版方向与可审核提示词 | 是 |
| P0 | 五图生成 | 以20:3连续画布整体生成并精确裁切五张4:3图片 | 是 |
| P0 | 可控修改 | 改字、换图、改色、重新裁切、单项重做、整套换风格 | 是 |
...
| P1 | 模板与品牌资产 | 餐饮子类模板、品牌色、字体与Logo锁定 | 后续增强 |
| P1 | 风格案例 | 按菜系、客单价、消费场景筛选并复用风格参数 | 后续增强 |
...
| --- | --- | --- | --- | --- |
| 创建项目 | 用户为餐饮门店发起做图 | 输入店名，选择抖音/美团 | 创建餐饮项目草稿并显示素材补充入口 | 是否双平台生成 |
| 上传素材 | 提供真实经营资料 | 上传菜单、门头、菜品、环境、Logo并标记角色/优先级 | 展示上传进度、用途和质量提示 | 素材是否足够、主视觉是哪张 |
| AI分析与补问 | 少填信息又不漏关键信息 | 回答缺口问题或补传指定图片 | 显示已获取、缺失及提问理由 | 是否已覆盖店名、定位、主推内容 |
| 事实确认 | 确保内容真实 | 核对门店事实卡中的店名、菜品、价格、地址、卖点及证据 | 标记已确认字段 | 是否允许开始策划 |
| 表达选择 | 确定内容口吻 | 接受系统推荐或选择品牌官方/老板推荐/食客种草 | 预览表达目标和适用场景 | 是否使用该视角 |
| 方案确认 | 确认整套表达 | 查看五图大纲、素材分配、排版、Logo方向及生图提示词 | 可补充主推菜品与风格要求 | 是否生成门店视觉包 |
| 生成 | 得到第一版 | 等待，可取消或离开 | 分阶段进度、单项结果陆续出现 | 失败项是否重试 |
| 编辑 | 调整成可用版本 | 换图、改字、改色、重新裁切、单项重做、选择Logo | 只重算受影响资产，保留整套连续性 | 是否达到可导出状态 |
...
4. 非餐饮项目在MVP中明确提示暂未开放，不进入通用生成链路。
...
6. 每张素材必须可标记角色：`门头`、`菜单`、`招牌菜`、`其他菜品`、`环境`、`Logo`、`资质`、`其他`。
7. 用户可拖拽调整同角色素材优先级，也可点击“设为主视觉”；系统不得只靠上传顺序猜测素材用途。
8. AI可建议素材角色，但用户确认值为准；角色变化只使依赖该素材的下游方案和资产失效。
...
| 素材角色与优先级 | AI建议+用户标记 | 是 |
| 主推菜品/套餐 | 菜单、菜品图+用户选择 | 是 |
| 表达视角 | 用户选择/系统建议 | 是，可采用默认值 |
...
通过充分度门槛后，系统将分散识别结果汇总为唯一可确认的“门店事实卡”。事实卡至少包含店名、餐饮子类、门店定位、主推菜品/套餐、价格、核心卖点、品牌色、已有Logo状态、素材角色及来源证据。

1. 图片生成、五图文案、Logo和品牌介绍图只能读取最新已确认事实卡。
2. AI提取的卖点以候选形式呈现，用户必须选择一个主卖点，可补充次卖点。
3. 每个强事实显示来源图片或用户补充来源；无证据字段显示“待补充”，不能伪装成已识别。
4. 事实卡修改生成新版本，并按依赖关系失效下游方案、生成资产与校验结果。

### 6.4 五图内容策划

策划前先选择“表达视角”。MVP提供三个受控选项：

| 表达视角 | 适用目标 | 文案边界 |
| --- | --- | --- |
| 品牌官方 | 稳定传达门店定位与套餐信息 | 克制、确定，不使用虚构第一人称经历 |
| 老板推荐 | 强调招牌菜、做法与经营理念 | 仅允许使用用户确认的老板口吻和真实经营信息 |
| 食客种草 | 强调场景感、口味体验与推荐理由 | 属于营销表达，不得伪造真实顾客身份、评价或消费经历 |

系统可根据素材和目标平台推荐默认视角，但必须展示原因并允许用户切换。表达视角只影响内容组织和语气，不得改变已确认事实。
...
| 素材角色/优先级 | 重新计算素材分配；仅失效引用发生变化的方案和资产 |
| 表达视角 | 五图文案、品牌介绍文案及相关策划失效；确认事实不失效 |
...
1. 有发布权限的用户选择目标门店和平台账号，账号持有人按平台要求完成授权；授权令牌加密保存并支持撤销。
...
1. 首期交付统一的响应式Web/WAP产品；桌面端与手机端共享业务能力、账号体系和项目状态，不建立BD专属版本。
...
| `generate_visual_asset` | 资产类型、提示词、参考图、指定供应商 | 图片资产、供应商/版本/耗时/费用元数据 | 单次调用一家外部模型并写资产 | 已授权项目内自动 |
| `retry_visual_asset` | 失败任务、错误分类、重试次数 | 千问重试或豆包切换决策 | 可能产生第二次模型费用并写切换记录 | 仅可重试错误自动；内容安全/事实缺失不得切换绕过 |
...
```text
你是餐饮团购视觉策划Agent。你的目标是基于用户授权的真实餐饮门店素材，
...
11. 图片生成默认调用千问；只有千问发生已分类的可重试失败时，下一次重试才可切换豆包Seedream 5.0 Lite；一次调用不得同时请求两家。
```

### 8.6 模型分工

【已确认】避免一个模型包办全部工作，并将供应商路由外置配置：

| 能力 | 建议模型/服务 | 选型重点 |
| --- | --- | --- |
| OCR与版面识别 | 阿里云百炼`qwen-vl-ocr`系列，生产锁定具体版本 | 中文菜单、价格、坐标和置信度 |
| 经营信息归纳 | 阿里云百炼视觉/文本模型，生产锁定具体版本 | 结构化JSON输出、禁止猜测 |
| 五图与品牌策划 | 文本模型 | 行业知识、营销表达、可追溯事实 |
| 图片生成/编辑 | 默认千问图片模型；千问调用失败后的下一次重试可切`Doubao-Seedream-5.0-lite` | 参考图一致性、局部编辑、商业质感、失败切换 |
| 中文文字落图 | 确定性Canvas/排版服务 | 文字准确、字体版权、安全区 |
| 视觉质量审核 | 独立视觉评估模型+规则 | 乱码、重复、事实偏差和布局风险 |

图片生成采用“单次单供应商”：一次任务只调用千问或豆包，不并行双生成。默认千问；只有出现明确的供应商失败、超时或可重试错误后，重试节点才允许切换豆包。因内容安全、事实缺失或参数错误导致的失败不得通过换供应商绕过。两家调用均记录模型版本、原因、耗时、费用和输出资产；具体千问型号、豆包版本ID、超时阈值和项目成本上限仍需PoC锁定。

### 8.7 知识与规则库

1. 平台规则：平台、版本、生效时间、图片数量、比例、分辨率、大小、格式、安全区、禁止内容；
2. 行业五图范式：MVP仅餐饮，并按火锅、烧烤、中餐、快餐、饮品等子类逐步建设；
...
```mermaid
flowchart TD
    L1[① 交互层<br/>统一响应式Web/WAP<br/>后续微信小程序；同步确认+异步任务通知]
...
贯穿数据流：用户创建项目并上传素材 → API创建持久化任务 → Worker完成识别并触发缺口追问 → 用户确认事实和Prompt → Workflow生成20:3母版与品牌资产 → 确定性排版/裁切 → 规则、视觉、安全三重校验 → 有权限用户确认发布 → 平台适配器提交并回写状态；每个节点写入任务journal、资产版本、审计日志和指标。
...
- `Web/WAP Frontend`：统一项目工作台、素材/事实确认、资产编辑和发布状态页面；不同账号仅按权限显示可用操作，不分叉产品通道；
...
| SourceAsset | asset_id、type、semantic_role、priority、is_hero、url、hash、quality、authorization、metadata |
| StoreFactCard | fact_card_id、project_id、industry、positioning、hero_item、selling_points、expression_view、status、version |
...
| GeneratedAsset | asset_id、type、panel_index、version、source_refs、provider、model_version、fallback_reason、generation_meta、status |
...
| 容量 | 单账号并发项目数、系统并发任务数和队列积压阈值在压测后确定；超阈值排队而非丢任务 |
...
- 核心样本60%：清晰菜单、标准门头、火锅/烧烤/中餐/快餐/饮品等餐饮子类；
...
| 安全 | 操作者误选门店或账号导致内容发布到错误主体 | P0 | 账号/门店强展示、最终预览、二次确认、权限审计与幂等提交 |
| 成本 | 多轮重做导致模型成本不可控 | P1 | 生成前确认、额度、缓存和单节点重做 |
| 体验 | 通用模板导致行业不匹配 | P1 | 先聚焦餐饮，再扩行业技能与模板 |
| 体验 | 隐式使用上传顺序导致主视觉选错 | P0 | 显式素材角色、主视觉与优先级，用户确认后再策划 |
| 成本/稳定性 | 双供应商同时调用造成重复费用和结果分叉 | P1 | 单次单供应商；默认千问，仅在可重试失败后切豆包并记录原因 |
...
| --- | --- | --- | --- | --- | --- |
| R-01 | 产品 | 项目名与MVP首发行业 | P1 | 项目名为团绘AI；MVP仅餐饮 | 已确认 |
...
| R-05 | 后端 | 生图供应商路由已定，具体型号、超时、并发和成本上限未锁定 | P1 | 默认千问，失败后可切豆包Seedream 5.0 Lite；完成PoC与成本测算 | 部分确认 |
...
| R-17 | 架构 | 单账号并发量、队列容量和模型预算未确定 | P1 | 用真实任务压测后确定限流、队列和项目预算 | 待确认 |
| R-18 | 平台 | “直接发布”缺少已验证的双平台接口与应用权限 | P0 | 立项首周完成接口/资质Spike，至少锁定一个真实闭环 | 待确认 |
| R-19 | 安全/业务 | 项目操作者与平台账号持有人谁完成最终发布确认尚未确定 | P0 | 默认账号持有人确认；委托他人操作时需可验证、可撤销授权 | 待确认 |
| R-20 | 运维 | 任务journal、Trace、告警和人工接管SOP尚未指定负责人 | P1 | 上线前确定值班、告警阈值、恢复和回滚负责人 | 待确认 |
| R-21 | 产品/设计 | 万相营造式任务化入口是否适用于本地生活 | P1 | 以“门店视觉包”为总交付，增加素材角色、表达视角和项目内局部返工，不扩通用工具箱 | 已确认 |
...
2. **发布确认权限**：项目操作者可凭授权直接发布，还是必须由平台账号持有人完成最后一次确认？
3. **名称注册检查**：团绘AI的商标、域名、小程序名称和应用市场重名筛查尚未完成。

其次需要业务补充或确认：

- 两个平台当前官方上传规则原文或后台截图；
- 10—30组餐饮真实脱敏菜单/门头/菜品样本；若从公开网络采集，只用于内部PoC并保留来源，不直接作为训练集或商业模板；
...
创作者完成上述关键确认后，下一版本将补充：

1. 完成团绘AI名称注册检查并最终锁定MVP边界；
2. 抖音/美团平台规则表及规则版本；
3. 页面级原型与交互说明；
4. 每个工作流节点的输入输出JSON Schema；
5. 千问/豆包A/B样本评测、失败切换测试、成本与时延选型；
6. 测试数据集、效果基线和上线门槛；
7. 研发拆解与排期；
8. 评审结论和阻断项负责人。
